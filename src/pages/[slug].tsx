...
export async function getStaticPaths() {
  ...
  const categories = await extractors.categories.fetchAll();
...
export async function getStaticProps({ locale, params }) {
  ...
  const [i18n, category, tools] = await Promise.all([
    serverSideTranslations(locale, ["common", "categories"]),
    extractors.categories.fetchOne({ slug }),
    extractors.tools.fetchAll({ slug }),
  ]);
